...
function openUsb () {   //搜索底座存不存在，不存在就去展示
    ...
    if (windowOpen) {     //如果没有点击退出
        if (!flog) {
            ...
            console.log('找到了设备', deviceUSB, `${enterUpgradeFlog ? '升级状态' : '非升级状态'}`);
...
menu.append(new MenuItem({
    ...
    submenu: [
        ...
        },
        {
            ...
            // accelerator: 'Cmd+Q',
            label: 'Hide Mella',
            click: () => { console.log('点击了hide'); }
        },
        {
            role: 'hideOthers',
            // accelerator: 'Cmd+Q',
            // label: 'Quit Mella',
            click: () => { console.log('点击了hideOthers'); }
        },
...
app.on('ready', () => {
    ...
    autoUpdater.on('error', (err) => {
        ...
        mainWindow.webContents.send('uploadMessage', { payload: statusMessage.error, output: err })
        //升级错误提示
        // dialog.showErrorBox('Error:', err === null ? 'unknown' : `${err}`)
    })
    autoUpdater.on('update-available', (val) => {
        console.log('我要开始更新了', val);
        mainWindow.webContents.send('uploadMessage', { payload: statusMessage.updateAva, output: val })
        dialog.showMessageBox({
            ...
            .then(res => {
                ...
                if (res.response === 0) {
                    ...
                    mainWindow.webContents.send('uploadMessage', { payload: statusMessage.cencleUpdate, output: null })
                }
            })
            .catch(err => {
                console.log(err);
    ...
    autoUpdater.on('update-downloaded', (val) => {
        ...
        mainWindow.webContents.send('uploadMessage', { payload: statusMessage.downloadSuccess, output: val })
        dialog.showMessageBox({
            type: 'info',
            title: 'Install',
            ...
            .then(res => {
                ...
                if (res.response === 0) {
                    ...
                    mainWindow.webContents.send('uploadMessage', { payload: statusMessage.cencleInstall, output: null })
                }
            })
            .catch(err => {
                console.log(err);
...
ipcMain.on('enterUpgrade', (event, data) => {
    ...
    try {
        ...
        mainWindow.webContents.send('uploadBaseInfo', { status: 'error1', data: '' })
    }
...
ipcMain.on('updateBase', (event, data) => {
    ...
    let fileUrl = `${path.join(__dirname, './build/2_no_aes_ota_MellaPro_Charger(1).bin')}`
    if (data.state === 'reset') {
    ...
    if (!file) {
        mainWindow.webContents.send('uploadBaseInfo', { status: 'error', data: 'Upgrade file not found' })
    } else {
        let arr = []
...
function sendUpload (params) {
    ...
    if (sendNum < dataArr.length) {
        ...
        mainWindow.webContents.send('uploadBaseInfo', { status: 'normal', data: `Upgrade progress：${progress}%`, progress })
        // if (`${progress}` !== '100.00') {
